...
if (!window.customElements.get('swiper-dots')) {
	window.customElements.define('swiper-dots', PageDots)
}
...
import { timeline } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
import {
	...
	stagger,
} from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
import { stagger as stagger2 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
var HeightChangeListener = class extends HTMLElement {
	constructor() {
		super()
	...
	connectedCallback() {
		if (!window.ResizeObserver) {
			document.documentElement.style.setProperty(
				`--ul-${this.getAttribute('variable')}-height`,
...
if (!window.customElements.get('height-change-listener')) {
	window.customElements.define('height-change-listener', HeightChangeListener)
}

// js/common/behavior/safe-sticky.js
import { inView as inView2 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
var _resizeObserver,
...
var template = `
  <style>
	...
	s {
	  ...
	  pointer-events: none;
	  background-image:
		var(--ul-scroll-shadow-top, linear-gradient(to bottom, rgb(var(--ul-background)), rgb(var(--ul-background) / 0))),
		var(--ul-scroll-shadow-bottom, linear-gradient(to top, rgb(var(--ul-background)), rgb(var(--ul-background) / 0))),
		var(--ul-scroll-shadow-left, linear-gradient(to right, rgb(var(--ul-background)), rgb(var(--ul-background) / 0))),
		var(--ul-scroll-shadow-right, linear-gradient(to left, rgb(var(--ul-background)), rgb(var(--ul-background) / 0)));
	  background-position: top, bottom, left, right;
	  background-repeat: no-repeat;
	  background-size: 100% var(--ul-top, 0), 100% var(--ul-bottom, 0), var(--ul-left, 0) 100%, var(--ul-right, 0) 100%;
	}
  </style>
  <slot></slot>
  <s></s>
...
var Updater = class {
	...
	update(targetElement, style) {
		...
		const maxSize = style.getPropertyValue('--ul-scroll-shadow-size')
			? parseInt(style.getPropertyValue('--ul-scroll-shadow-size'))
			: 0
		const scroll2 = {
...
import { scroll } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
var VisibilityProgress = class extends HTMLElement {
	connectedCallback() {
		scroll(
			info => {
				this.style.setProperty(
					'--ul-visibility-progress',
					info.y.progress,
				)
			},
			{
...
import { animate as animate3 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
import { animate as animate4 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"

// js/common/overlay/dialog-element.js
import { FocusTrap, Delegate } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
		...
		switch (this.openFrom) {
			case 'left':
				return document.dir === 'ltr'
					? [
							'inset(0 100% 0 0 round var(--ul-rounded-sm))',
							'inset(0 0 0 0 round var(--ul-rounded-sm))',
					  ]
					: [
							'inset(0 0 0 100% round var(--ul-rounded-sm))',
							'inset(0 0 0 0 round var(--ul-rounded-sm))',
					  ]
			case 'right':
				return document.dir === 'ltr'
			...
			case 'bottom':
				return [
					'inset(100% 0 0 0 round var(--ul-rounded-sm))',
					'inset(0 0 0 0 round var(--ul-rounded-sm))',
				]
			case 'top':
				return [
					'inset(0 0 100% 0 round var(--ul-rounded-sm))',
					'inset(0 0 0 0 round var(--ul-rounded-sm))',
				]
		}
	}
...
var Popover = class extends DialogElement {
	...
	_showTransition(animate11 = true) {
		...
		if (window.matchMedia('screen and (max-width: 999px)').matches) {
			...
			let spacingBlockValue = 'var(--ul-popover-anchor-block-spacing)',
				spacingInlineValue = 'var(--ul-popover-anchor-inline-spacing)'
			this.style.left =
...
if (!window.customElements.get('theme-popover')) {
	window.customElements.define('theme-popover', Popover)
}

import { Delegate as Delegate2 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
var FreeShippingBar = class extends HTMLElement {
	static get observedAttributes() {
		return ['first-threshold', 'second-threshold', 'total-price']
	}
	...
	async connectedCallback() {
		// Convert thresholds to account for currency rate
		this.firstThreshold = Math.round(
			this.getAttribute('first-threshold') * (Shopify.currency.rate || 1)
		)
		this.secondThreshold = Math.round(
			this.getAttribute('second-threshold') * (Shopify.currency.rate || 1)
		)
	...
	async attributeChangedCallback(attribute, oldValue, newValue) {
		...
		const progress = Math.min(this.totalPrice / this.secondThreshold, 1)
		progressBarElement.style.setProperty('--ul-progress', progress.toString())

				// Store previous state
		const wasBelowFirstThreshold = this.previousPrice < this.firstThreshold;
		const wasBelowSecondThreshold = this.previousPrice < this.secondThreshold;

		this.previousPrice = this.totalPrice;

		// Check thresholds and update cart accordingly
		if ( this.totalPrice >= this.secondThreshold && wasBelowSecondThreshold ) {
			await this._addFirstProductToCart();
			await this._handleSecondThresholdReached();
			messageElement.innerHTML = this.getAttribute('reached-message');
		} else if ( this.totalPrice >= this.firstThreshold && wasBelowFirstThreshold ) {
			await this._addFirstProductToCart();
			// Show appropriate message based on current total after adding product
			if ( this.totalPrice >= this.secondThreshold ) {
				// is this check useful?
				messageElement.innerHTML = this.getAttribute('reached-message');
			} else {
				const remaining = this.secondThreshold - this.totalPrice;
				const message = this.getAttribute('second-message')
					.replace('$$$', formatMoney(remaining))
					.replace('@@@', `<strong>${this.getAttribute('second-benefit')}</strong>`);
				messageElement.innerHTML = message;
			}
		} else {
			// Update messages based on current cart total
			if (this.totalPrice >= this.secondThreshold) {
				messageElement.innerHTML = this.getAttribute('reached-message');
			} else if (this.totalPrice >= this.firstThreshold) {
				const remaining = this.secondThreshold - this.totalPrice;
				const message = this.getAttribute('second-message')
					.replace('$$$', formatMoney(remaining))
					.replace('@@@', `<strong>${this.getAttribute('second-benefit')}</strong>`);
				...
				const remaining = this.firstThreshold - this.totalPrice;
				const message = this.getAttribute('unreached-message')
					.replace('$$$', formatMoney(remaining))
					.replace('###', `<strong>${this.getAttribute('first-product')}</strong>`);
				messageElement.innerHTML = message;
			}
		}
	}

	async _addFirstProductToCart() {
		const productId = this.getAttribute('first-product-id')
		if (!productId) return

		try {
			await this._addProductToCart(productId)
		} catch (error) {
			console.error('Error adding first threshold product:', error)
		}
	}

	async _handleSecondThresholdReached() {
		const benefit = this.getAttribute('second-threshold-benefit')
		const secondProductId = this.getAttribute('second-product-id')

		try {
			switch (benefit) {
				case 'free_product':
					if (secondProductId) {
						await this._addProductToCart(secondProductId)
					}
					break
				...
				case 'free_shipping':
					await this._applyFreeShipping()
					break
			}
		} catch (error) {
			console.error('Error handling second threshold:', error);
		}
	}

	async _addProductToCart( productId ) {
		// First check if the product is already in cart
		const cartContent = await fetchCart;
		const existingItem = cartContent.items.find(item => item.id.toString() === productId.toString());
		...
		const response = await fetch(`${Shopify.routes.root}cart/add.js`, {
			...
			body: JSON.stringify({
				...
				sections: sectionsToBundle,
				//sections_url: `${Shopify.routes.root}variants/${this.id.value}`,
			}),
		} );
		...
		if ( response.ok ) {
			...
			document.documentElement.dispatchEvent(
				new CustomEvent('cart:change', {
					bubbles: true,
					detail: {
						baseEvent: 'free-shipping-bar:product-added',
						cart: cartContent,
					},
				}),
			)
	...
	async _applyDiscount() {
		const discountAmount = this.getAttribute('discount-amount')
		if (!discountAmount) return

		// Here you would implement the logic to apply the discount
		// This might involve adding a discount code or updating the cart
		// The exact implementation will depend on your store's setup
		console.log('Applying discount:', discountAmount)
	}

	async _applyFreeShipping() {
		// Here you would implement the logic to apply free shipping
		// This might involve adding a shipping discount or updating shipping rates
		// The exact implementation will depend on your store's setup
	...
	async _onCartChanged(event) {
		...
		const cartDiscount = event.detail['cart']['cart_level_discount_applications']
		...
		// Check if we need to remove benefits when total drops below thresholds
		if (this.totalPrice >= this.secondThreshold && newTotalPrice < this.secondThreshold) {
			// Remove second threshold benefits
			await this._removeSecondThresholdBenefits(event.detail.cart);
		}

		if (this.totalPrice >= this.firstThreshold && newTotalPrice < this.firstThreshold) {
			// Remove first threshold benefits
			await this._removeFirstThresholdBenefits(event.detail.cart);
		}

		this.totalPrice = newTotalPrice;
	}

	async _removeFirstThresholdBenefits(cart) {
		const firstProductId = this.getAttribute('first-product-id');
		if (!firstProductId) return;

		// Find the benefit product in cart
		const itemToRemove = cart.items.find(item => item.id.toString() === firstProductId.toString());
		if (itemToRemove) {
			await this._removeItemFromCart(itemToRemove.key);
		}
	}

	async _removeSecondThresholdBenefits(cart) {
		const benefit = this.getAttribute('second-threshold-benefit');
		const secondProductId = this.getAttribute('second-product-id');
		...
		switch (benefit) {
			case 'free_product':
				if ( secondProductId ) {
					const itemToRemove = cart.items.find(item => item.id.toString() === secondProductId.toString());
					if (itemToRemove) {
						await this._removeItemFromCart(itemToRemove.key);
					}
				}
			...
			case 'discount':
				await this._removeDiscount();
				break;

			case 'free_shipping':
				await this._removeFreeShipping();
				break;
		}
	}

	async _removeItemFromCart( itemKey ) {
		// Get existing LineItemQuantity class and call its static method directly
		const g = new LineItemQuantity()
		await g._changeLineItemQuantity( itemKey, 0 );
	}

	async _removeDiscount() {
		// Here you would implement the logic to remove the discount
		// This might involve removing a discount code or updating the cart
		console.log('Removing discount...');
	}

	async _removeFreeShipping() {
		// Here you would implement the logic to remove free shipping
		// This might involve removing a shipping discount or updating rates
		console.log('Removing free shipping...');
	}
}
...
import { animate as animate5 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
import { Delegate as Delegate3 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
var ProductItem = class extends HTMLElement {
	constructor() {
		super()
		this._delegate = new Delegate3(this)
		this.addEventListener(
			'pointerover',
			() => ProductLoader.load(this.getAttribute('uri')),
			{ once: true },
		)
	}
	connectedCallback() {
	...
	async _getMatchingVariant(target) {
		this.product = await ProductLoader.load(this.getAttribute('uri'))
...
if (!window.customElements.get('product-item')) {
...
var BuyButtons = class extends HTMLElement {
	...
	_onCartError(event) {
	  ...
	  <svg role="presentation" focusable="false" width="18" height="18" class="offset-icon icon icon-error" style="--ul-icon-height: 18px" viewBox="0 0 18 18">
...
import { PhotoSwipeLightbox } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
var ProductGallery = class extends HTMLElement {
	...
	connectedCallback() {
		...
		this._pageDots = Array.from(this.querySelectorAll('swiper-dots'))
	...
	get photoswipe() {
		...
		const photoswipe = new PhotoSwipeLightbox({
			pswpModule: () => import("//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/photoswipe.min.js?v=12120499261991998211788584905"),
			bgOpacity: 1,
		...
		if (event.scale > 1.5) {
			this.dispatchEvent(
				new CustomEvent('lightbox:open', {
					bubbles: true,
					detail: {
						index: this.selectedIndexAmongVisible,
					},
				}),
			)
...
import { inView as inView4 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
var VideoMedia = class extends BaseMedia {
	connectedCallback() {
		...
		<svg part="play-button" fill="none" width="48" height="48" viewBox="0 0 48 48">
		  <path d="M48 24C48 10.745 37.255 0 24 0S0 10.745 0 24s10.745 24 24 24 24-10.745 24-24Z" fill="#0A1220"/>
		  <path d="M18.578 32.629a.375.375 0 0 1-.578-.316V15.687c0-.297.328-.476.578-.316l12.931 8.314c.23.147.23.483 0 .63L18.578 32.63Z" fill="#EAF0F7"/>
		  <path d="M24 .5C36.979.5 47.5 11.021 47.5 24S36.979 47.5 24 47.5.5 36.979.5 24 11.021.5 24 .5Z" stroke="#EAF0F7" stroke-opacity=".12"/>
		</svg>
	  `),
			)
		}
...
import { timeline as timeline6 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
import { animate as animate8 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
import { animate as animate9 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
...
class CountdownTimer extends HTMLElement {
	...
	constructor() {
		...
		this.shadowRoot.querySelector('h4[data-text]').innerHTML =
			this.dataset.timerText
	}
	...
	updateCountdownTimer() {
		...
		const textElm = this.shadowRoot.querySelector('h4[data-text]')
		...
		if (timeleft <= 0) {
			...
			textElm.innerHTML = this.dataset.timerOverText
			return
		}
...
if (!window.customElements.get('moving-text')) {
	window.customElements.define('moving-text', MarqueeText)
}

// js/theme.js
import { Delegate as Delegate4 } from "//sleep-and-wellbeing-bpw11isf.myshopify.com/cdn/shop/t/11/assets/app.min.js?v=11786474696640468811788584905"
;(() => {
